...
export function getEscrowFromCollectionName(
  endpoint: string,
...
export function getCreatorFromCustomContract(
  endpoint: string,
...
export const getTraitValueByKey = (
  traitKey: string,
...
export const getFiltersFromUrl = ({
  ...
  const validFilterKeys = Object.keys(filtersToApply).filter((f) =>
    collectionFilters?.some(
      (mdf) => mdf.name === f && mdf.values.includes(filtersToApply[f])
    )
  )
...
export const encodeFiltersToUrl = (filtersToEncode: {
...
export const authApiRequest = async (
  ...
  const authBearer = localStorage.getItem(
  ...
  try {
    ...
    if (e.status == 401) {
      try {
        ...
        const signMessage = await wallet.wallet.signMessage(
        ...
        toast.info(
          "Authentication Complete! \n We initiated your request again",
          {
            position: "bottom-left",
            autoClose: 5000,
          }
...
export const makeUrlStringFromObject = (data: any) => {
  var urlString = ""
  Object.keys(data).forEach(function (key) {
    return (urlString += key + "=" + data[key] + "&")
  })
...
export const dayDifference = (payload: number): number => {
  const today = new Date()
  const createdOn = new Date(payload)
...
export const formatToHyphen = (payload: string): string => {
  if (payload) {
    return  payload.split(" ").join("-").toLocaleLowerCase()
  }

  return payload
}

export const formatToSpaces = (payload: string): string => {
  if (payload) {
    return  payload.includes(' ') ? payload.toLocaleLowerCase() : payload.split("-").join(" ").toLocaleLowerCase()
  }
