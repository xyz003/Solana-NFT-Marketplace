...
import SoloCreationContext, {
  SoloCreationStep,
  stepList,
} from "../../contexts/solo-creation"
import {
...
export const SoloCreateStepper: React.FC<SoloCreateStepperProps> = ({
  data,
}) => {
  // hooks
  const {
  ...
  const [hasProfileBannerChanged, setHasProfileBannerChanged] =
    useState<boolean>(false)
  const [outroDisplayedImage, setOutroDisplayedImage] = useState<string>("")
  ...
  const startingTheme = JSON.stringify(SOLO_THEMES["default"])
  const [themeSelected, setThemeSelected] = useState<string>(startingTheme)
  ...
  async function sendEmailVerificationCode(email: string) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      await fetch(BASE_URL_SOLO_GET_CODE, {
        method: "POST",
        headers: {
  ...
  async function verifyEmailWithCode(code: string) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      await fetch(BASE_URL_SOLO_VERIFY_CODE, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  async function handleDetailsSubmit() {
    ...
    if (!hasEmailChanged) {
      ...
      try {
        ...
        toast.error(
          <Notification title="Error submitting details" description={error} />
        )
        setIsLoading(false)
      }
    }
  ...
  async function handleCodeInputKeyDown(event: any) {
    const { key, target } = event
    const { value } = target

    const previousInput = target.previousElementSibling
    const isPaste = key === "v" && event.ctrlKey
    ...
    if (isBackspace) {
      if (previousInput) {
        updateCodeValue(target.dataset.codePosition, "")
        previousInput.focus()
      } else {
        updateCodeValue(target.dataset.codePosition, "")
      }
  ...
  function onCodePaste(event: any) {
    ...
    const isValid = pastedData.length === 6 && /^-?\d+$/.test(pastedData)
  ...
  useEffect(() => {
    ...
    if (profileImage && !isImageUrl) {
      ...
      reader.onload = () => {
        setOutroDisplayedImage(reader.result as string)
      }
      reader.readAsDataURL(profileImage as unknown as File)
    }

    if (profileImage && isImageUrl) {
      setOutroDisplayedImage(profileImage)
    }
  }, [profileImage])
  ...
  useEffect(() => {
    if (data) {
      ...
      setThemeSelected(
        data.theme === "default"
          ? JSON.stringify(SOLO_THEMES["default"])
          : data.theme
      )
    }
  }, [data])
  ...
  return (
    <section
      className={`my-20 ${
        isLoading === true ? "pointer-events-none cursor-not-allowed" : ""
      }`}
    >
      {currentStep === "details" && (
        <div ref={stepsRefs.details}>
          ...
          >
            ...
            <StepperInput
              ...
              onBlur={async (e: any) => {
                const descriptionValidation = await Validate.description(
                  e.target.value
                )
                setIsDescriptionValid(descriptionValidation.isValid)
                setDescriptionError(descriptionValidation.error)
            ...
            />
            <div className="flex flex-row flex-wrap gap-10 lg:flex-nowrap lg:gap-5">
              <StepperInput
                name="discord"
                type="text"
                ...
                onBlur={async (e: any) => {
                  const discordValidation = await Validate.discord(
                    e.target.value
                  )
                  setIsDiscordValid(discordValidation.isValid)
                  setDiscordError(discordValidation.error)
              ...
              <StepperInput
                ...
                onBlur={async (e: any) => {
                  const twitterValidation = await Validate.twitter(
                    e.target.value
                  )
                  setIsTwitterValid(twitterValidation.isValid)
                  setTwitterError(twitterValidation.error)
              ...
              <StepperInput
                ...
                onBlur={async (e: any) => {
                  const instagramValidation = await Validate.instagram(
                    e.target.value
                  )
                  setIsInstagramValid(instagramValidation.isValid)
                  setInstagramError(instagramValidation.error)
            ...
            <>
              ...
              >
                <input
                  ...
                  onChange={(e: any) => {
                    updateCodeValue(
                      e.target.dataset.codePosition,
                      e.target.value
                    )
                    handleCodeInputKeypress(e)
                  }}
      ...
      {currentStep === "customization" && (
        <div
          ref={stepsRefs.customization}
          className="flex items-center flex-col"
        >
          <h1 className="font-semibold text-xl">profile image & banner</h1>
          <p className="font-normal text-sm text-gray-500 my-3">
            You can upload a profile picture and a banner to be displayed on
            your profile.
          </p>
          ...
          <p className="font-normal text-sm text-gray-500 mt-3 mb-10 text-center">
            In order to make your profile unique, you can choose a theme from
            our curated list of presets or tune every single color to your
            liking. Go crazy! ^^
          </p>
          <ThemeSelector
      ...
      {currentStep === "outro" && (
        <div ref={stepsRefs.outro} className="flex items-center flex-col">
          ...
          {outroDisplayedImage ? (
            <img
              src={outroDisplayedImage}
              alt={`${username} profile image`}
              className="rounded-full h-48 w-48 mx-auto my-10 object-cover"
            />
          ) : (
            <div className="h-48 w-48 mx-auto my-10 bg-gray-800 rounded-full flex items-center justify-center">
              <h1 className="text-gray-700 font-semibold text-2xl">SOLO</h1>
            </div>
          )}
        ...
        {!isLastStep() && (
          <>
            ...
            <button
              ...
              className={
                "text-sm py-4 px-12 bg-blue-600 text-white font-semibold rounded-md transition ease-in-out hover:bg-blue-800" +
                (isLastStep()
                  ? "pointer-events-none blur-3xl cursor-not-allowed opacity-5"
                  : "")
              }
            >
