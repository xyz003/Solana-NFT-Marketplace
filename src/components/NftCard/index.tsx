...
export const NftCard: React.FC<NftCardProps> = ({
  ...
  const walletParam = wallet ? `&wallet=1` : ``

  const cacheFallback = (parentNode: any) => {
    setCacheFailed(true)
  }

  const collectionPath = offer?.collectionName
  ...
  const copyLink = () => {
    ...
    navigator.clipboard.writeText(
      `${window.location.origin}${ROUTES.ITEM}/${collectionPath}/${offer.mint}?pk=${offer.escrowPubkeyStr}${walletParam}`
    )
    setTimeout(() => {
      setCopied(false)
    }, 2000)
  ...
  const goToNFT = (e: any) => {
    ...
    if (collName?.split("#")[0] == "solo") {
      if (Boolean(e.ctrlKey || e.metaKey || e.button === 1)) {
        window.open(
          `${window.location.origin}${ROUTES.SOLO_ITEM}/${offer.mint}${walletParam}`
        )
      } else {
      ...
      if (Boolean(e.ctrlKey || e.metaKey || e.button === 1)) {
        window.open(
          `${window.location.origin}${ROUTES.ITEM}/${formatToHyphen(
            collectionPath
          )}/${offer.mint}?pk=${offer.escrowPubkeyStr}${walletParam}`
        )
      } else {
        history.push(
          `${ROUTES.ITEM}/${formatToHyphen(collectionPath)}/${offer.mint}?pk=${offer.escrowPubkeyStr
          }${walletParam}`
        )
      }
    }
  ...
  return (
    ...
    >
      <LazyLoad height={200} offset={100}>
        {offer.metadata?.animation_url &&
          (category === "html" || animationUrlExt === "html") ? (
          <HTMLContent
            animationUrl={offer.metadata?.animation_url}
          ></HTMLContent>
        ) : (
        ...
        <div className="flex-initial relative">
          ...
          <div
            className={`px-4 pb-2 flex justify-between items-end ${isToken && window.location.pathname.includes("/wallet")
              ? "hidden"
              : ""
              }`}
          >
            <p className="font-bold text-xl -ml-2 leading-none">
              ...
              >
                {isFavorite ? (
                  ...
                  <div className="inline-flex items-center text-xxs transition 50 ease-in-out">
                    ...
                    <span className="relative top-px transition 150 ease-in-out"></span>
                  </div>
                )}
              </button>
            </div>
