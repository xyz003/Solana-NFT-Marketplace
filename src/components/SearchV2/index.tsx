...
import { classNames, formatToHyphen, mapObjectQueryParams, removeNullValuesFromObject } from "../../utils";
  ...
  collectionDropdown.push(
      {
        ...
        options: topCollections.map((topCollection: Collection) => {
          return {
            value: `${ROUTES.COLLECTIONS}/${formatToHyphen(topCollection?.name)}`,
...
const selectStyles = { ...reactSelectCustomStyles, ...darkTheme };
  ...
  return (
    ...
    onCreateOption={(searchString)=>history.push(`${ROUTES.SEARCH_RESULTS}?q=${formatToHyphen(encodeURIComponent(searchString))}${searchActiveTab!=null ? "&tab="+searchActiveTab:""}`)}
