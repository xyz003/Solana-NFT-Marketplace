...
export const WalletView = () => {
  ...
  useEffect(() => {
    const domain = wallet?.domainNames && wallet?.publicKey
      ? getDomainList(wallet?.domainNames)
      : wallet?.publicKey
        ? `${shortenAddress(wallet?.publicKey.toString())}`
        : "";
    setDomains(domain);
  }, [wallet?.domainNames, wallet?.publicKey, connection])
  ...
  return (
    <Page title="Your Wallet | DigitalEyes">
      <div className="d-flex">
        <SideBar setStatus={setStatus} menuCollapse={menuCollapse} setMenuCollapse={setMenuCollapse} subTitle={domains} />
        <div className={`pt-10 px-4 sm:px-6 lg:px-8 wallet ${menuCollapse ? 'normal-width' : 'collapsed-width'}`}>
          <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <Switch test={index}>
              ...
              <BaseDiv value={3}>
                {wallet?.publicKey && (
                  <ActivitiesTable key={wallet?.publicKey?.toString()} />
                )}
              </BaseDiv>
