...
export const ResourceList = ({ category = "" }) => {
  ...
  useEffect(() => {
    const getPosts = async () => {
      try {
        var res = await fetch(
          `https://blog.digitaleyes.market/wp-json/wp/v2/posts?category_slug=${category.toLowerCase()}&_embed`
        )

        if (category.toLowerCase() == "guides") {
          res = await fetch(
            `https://blog.digitaleyes.market/wp-json/wp/v2/posts?category_slug=Buyer/Seller%20Guides&_embed`
          )
        }
  ...
  return (
    <Page
      className="md:max-w-7xl mx-auto sm:px-6 lg:px-8 py-16"
      title="Resource Item"
    >
        ...
        <div className="mt-5 max-w-lg mx-auto lg:grid-cols-12 lg:max-w-none">
          <div className="text-center">
            <h2 className="text-3xl tracking-tight font-extrabold text-white sm:text-4xl">
              {category == "Creator Guides" && "Guides"}
              {category != "Creator Guides" && category}
            </h2>
            <p className="mt-3 max-w-2xl mx-auto text-xl text-gray-300 sm:mt-4"></p>
          </div>
          ...
          {posts.map((post: any, idx) => (
            ...
            <div
              className="pl-10 pt-6 pr-10"
              onClick={() => goToResource(post.slug, post.id)}
            >
              <div className=" w-full lg:max-w-full lg:flex">
                {post._embedded["wp:featuredmedia"] && (
                  <div
                    className="h-48 lg:h-auto lg:w-48 mr-3 flex-none bg-cover rounded-t lg:rounded-t-none lg:rounded-l text-center overflow-hidden"
                    style={{
                ...
                {!post._embedded["wp:featuredmedia"] && (
                  <div
                    ...
                    style={{
                      ...
                      backgroundImage: "url(/de-guide2.jpg)",
                    }}
                    title="blogImage"
                  ></div>
                )}
                ...
                <div className="bg-gray-900 bg-opacity-50 rounded-b lg:w-full lg:rounded-b-none lg:rounded-r p-4 flex flex-col justify-between leading-normal">
                  <div className="mb-8">
                    <a
                      href="#"
                      className="hover:underline text-sm text-blue-700 flex items-center"
                    >
                      {category}
                    </a>
                  ...
                  <div className="flex items-center">
                    <img
                      ...
                      src="/assets/logo/default-digitaleyes-icon.jpeg"
                      alt="DE"
                    />
                    <div className="text-sm">
                      <p className="text-gray-500 leading-none">Digital Eyes</p>
                      <p className="text-gray-500">
                        {post && dayjs(post.date).format("MMM D")}
                      </p>
                    </div>
                  </div>
                </div>
