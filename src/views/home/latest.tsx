...
export const LatestCollections = () => {
  ...
  const sortedCollections = collectionsWithEpoch
    .sort(
      (collectionA, collectionB) =>
        (collectionB?.publishedEpoch as number) -
        (collectionA?.publishedEpoch as number)
    )

  let latestCollections = []

  for (let collection of sortedCollections) {
    if (!collection.isDerivative && collection.verifeyed) {
      latestCollections.push(collection)
    }

    if (latestCollections.length == 10) {
      break;
    }
  }
